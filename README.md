...
* **极速编译执行**：复用共享挂载目录与常驻 JVM 运行时，判题任务到达后直接通过 `docker exec` 将编译执行指令投递到空闲 Worker，将容器启动时间压缩至 **接近 0ms**，单次判题平均耗时优化至 **100ms** 级别。
...
* **XXL-JOB 调度中心**：`http://127.0.0.1:8080/xxl-job-admin`（账号/密码：`admin`/`123456`）
