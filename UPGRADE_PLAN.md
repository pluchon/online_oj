...
| Spring AI Alibaba | 无 | **1.1.2.3**（BOM 已在根 pom 引入，阶段 3 使用） | 基于 Boot 3.5.10、Spring AI 1.1.2 构建；另有 `1.1.2.4-security-fix`，阶段 3 开工时核对其修复内容再决定是否跟进 |
...
| 方案 | 做法 | 风险 |
|---|---|---|
| A（推荐） | 服务端升级到 Nacos 3.0.x，与客户端同代 | 3.x 控制台默认占用 8080（本机 8080 为 Windows 保留端口，xxl-job-admin 已改用 18080）；需配置鉴权相关环境变量；配置数据需从旧库迁移并核对 |
| B | 在依赖管理中把 `nacos-client` 固定为 2.5.x，服务端不动 | 偏离 SCA 官方测试组合，后续排错成本高 |

已选 A（2026-09-22）：服务端 `nacos-server:v3.2.4`（3.3 不支持从 2.x 直接平滑升级），配置库新建为 `bitoj_nacos_v3`，旧库 `bitoj_nacos_local` 保留作回退；控制台映射到宿主机 18848，客户端鉴权保持关闭。
...
- 已定（2026-09-22）：ES、Kibana 与 IK 插件统一升到 8.18.8，与客户端同版本。
- 三者版本必须完全一致；题目索引为空时 friend 会从数据库重建。
...
B 端题目管理已接入 `tb_question_case`（2026-09-21 完成，用例随题目整体保存），功能 1 可直接复用该保存链路。
...
| 编号 | 事项 | 影响阶段 |
|---|---|---|
| Q1 | ~~Nacos 方案~~ 已定（2026-09-22）：方案 A，服务端 3.2.4 | 阶段 1 |
| Q2 | ~~ES 服务端是否升级~~ 已定（2026-09-22）：升到 8.18.8 | 阶段 1 |
